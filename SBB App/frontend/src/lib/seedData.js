...
// Clean initial state: no invalidated tickets
...
export async function initializeSeedData(baseTime = Date.now()) {
  console.log(
    "[SEED] Initializing seed data with baseTime:",
    new Date(baseTime).toLocaleString()
  );
  ...
  try {
    ...
    console.log(
      "[SEED] Verification: Total tickets in DB:",
      allSavedTickets.length
    );

    // 4. Create invalidated tickets (none in clean initial state)
    console.log("[SEED] Skipping invalidated tickets (clean initial state)...");
    ...
    console.log(
      `[SEED] Created ${SEED_TOKEN_PURCHASES.length} token purchases`
    );
    ...
    await new Promise((resolve) => setTimeout(resolve, 300));
    console.log(
      "[SEED] Sending third round of notifications to force refresh..."
    );
    notifyTabs("cryptographicKeys", "update");
    notifyTabs("userTickets", "update");
    notifyTabs("invalidatedTickets", "update");
